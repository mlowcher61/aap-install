...
On the RHEL target server (as root):
...
```bash
git clone https://github.com/mlowcher61/aap-install.git /home/ansible-svc/aap-install
chown -R ansible-svc:ansible-svc /home/ansible-svc/aap-install
cd /home/ansible-svc/aap-install
```
...
### 6. Create and encrypt your vault file
...
### 7. Add your SSH public key for the ansible-svc account

```bash
cat ~/.ssh/id_rsa.pub > playbooks/files/public_keys/ansible-svc
```

### 8. Run the setup playbook
...
- **Server FQDN** — the fully qualified domain name of this server
- **RHEL version** — 9 or 10 (default: 9)
